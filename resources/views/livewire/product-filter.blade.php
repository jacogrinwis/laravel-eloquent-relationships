...
<div class="container mx-auto max-w-7xl p-4">
    <div class="grid grid-cols-4 gap-12">
        <aside class="col-span-1 space-y-6 text-sm">
            @if (count($selectedCategories) > 0 || count($selectedColors) > 0 || count($selectedMaterials) > 0)
                <section>
            ...
            <section x-data="{ open: true }">
                ...
                <div x-show="open">
                    ...
                    @if ($categories->count() > 10)
                        ...
                        >
                            {{ $showAllCategories ? 'Show less' : 'Show more (' . ($categories->count() - 5) . ')' }}
                        </button>
                    @endif
                </div>
            </section>
                ...
                <div x-show="open">
                    ...
                    @if ($colors->count() > 10)
                        <button
                            ...
                            class="text-sm font-semibold text-blue-600"
                        >
                            {{ $showAllColors ? 'Show less' : 'Show more (' . ($colors->count() - 5) . ')' }}
                        </button>
                    @endif
                </div>
            </section>
            ...
            <section x-data="{ open: true }">
                <div class="mb-2 flex items-center justify-between">
                    <h4
                        ...
                        @click="open = !open"
                    >
                        <span>Materials ({{ $materials->count() }})</span>
                        <svg
                            class="h-5 w-5 transition-transform duration-200"
                            :class="{ 'rotate-180': open }"
                            viewBox="0 0 20 20"
                ...
                <div x-show="open">
                    ...
                    @if ($categories->count() > 10)
                        <button
                            ...
                            class="text-sm font-semibold text-blue-600"
                        >
                            {{ $showAllMaterials ? 'Show less' : 'Show more (' . ($materials->count() - 5) . ')' }}
                        </button>
                    @endif
                </div>
            </section>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-2 grid h-14 grid-cols-2 gap-2">
                                    ...
                                    <div class="grid items-end justify-end">
                                        <x-products.add-to-cart-button :disabled="$product->stock_status === 'out_of_stock' ? true : false" />
                                    </div>
                                </div>
